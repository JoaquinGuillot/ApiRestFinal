...
//PersonaServices servicio;
    ...
    public ResponseEntity<?> getOne(@PathVariable Long id){
        try{
            return ResponseEntity.status(HttpStatus.OK).body("Busqué esta persona por Id:" + personaServices.findById(id));
        }
        catch (Exception e){
            return ResponseEntity.status(HttpStatus.NOT_FOUND).body("{\"error\":\"Error, por favor intente más tarde\"}");
        }
    ...
    public ResponseEntity<?> save(@RequestBody Persona entity){
        ...
        try{
            return ResponseEntity.status(HttpStatus.OK).body("Grabé los datos anteriores " + personaServices.save(entity));
        }
        catch (Exception e){
    ...
    public ResponseEntity<?> update(@PathVariable Long id, @RequestBody Persona entity){
        ...
        try{
            return ResponseEntity.status(HttpStatus.OK).body("Actualicé los datos anteriores" + personaServices.update(id,entity));
        }
        catch (Exception e){
            return ResponseEntity.status(HttpStatus.BAD_REQUEST).body("{\"error\":\"Error, por favor intente más tarde\"}");
        }
    ...
    public ResponseEntity<?> delete(@PathVariable Long id){
        try{
            return ResponseEntity.status(HttpStatus.NO_CONTENT).body("Eliminé el registro " + personaServices.delete(id));
        }
        catch (Exception e){
            return ResponseEntity.status(HttpStatus.BAD_REQUEST).body("{\"error\":\"Error, por favor intente más tarde\"}");
        }
